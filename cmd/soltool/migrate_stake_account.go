...
func migrateStakeAccount(ctx *cli.Context) (err error) {
	defer func() {
		if err != nil {
			fmt.Println("err: ", err)
		}
		...
		rawTx, err := solTypes.CreateRawTransaction(solTypes.CreateRawTransactionParam{
			Instructions: []solTypes.Instruction{
				...
				multisigprog.CreateTransaction(
					MultisigProgramId,
					[]solCommon.PublicKey{migrateStakeInstruction.ProgramID},
					[][]solTypes.AccountMeta{migrateStakeInstruction.Accounts},
					[][]byte{migrateStakeInstruction.Data},
	...
	for i := 0; i < len(otherFeeAccount); i++ {
		...
		remainingAccounts := multisigprog.GetRemainAccounts([]solTypes.Instruction{migrateStakeInstruction})
		rawTx, err := solTypes.CreateRawTransaction(solTypes.CreateRawTransactionParam{
			Instructions: []solTypes.Instruction{
	...
	for {
		...
		if txInfo.DidExecute == 1 {
			...
			fmt.Printf("migrate not success yet, waiting...\n")
			retry++
			time.Sleep(3 * time.Second)
			continue
		}
...
func getMultisigTxAccountPubkeyForMigrate(baseAccount, programID solCommon.PublicKey, index int) (solCommon.PublicKey, string) {
	seed := fmt.Sprintf("initAccountForMigrate:%d", index)
