...
func (w *writer) processValidatorRedelegateTarget(m *core.Message) bool {
	...
	needRedelegateVals := make(map[string]types.Coin)
	...
	for _, dele := range deleRes.GetDelegationResponses() {
		...
		//filter validators no need redelegate
		if w.conn.validatorNoNeedRedelegate[strings.ToLower(valAddr.String())] {
			continue
		}
		needRedelegateVals[valAddr.String()] = dele.GetBalance()
	}
	if len(needRedelegateVals) == 0 {
		w.log.Warn("processValidatorRedelegateTarget no validators need redelegate")
		return true
	}
	for key, value := range needRedelegateVals {
		w.log.Info("GenMultiSigRawReDelegateTxWithTarget", "need relegate", key, "amount", value.Amount.String())
	}

	unSignedTx, err := client.GenMultiSigRawReDelegateTxWithTarget(
		poolAddr,
		needRedelegateVals,
		w.conn.validatorTargets[0])
	if err != nil {
		w.log.Error("GenMultiSigRawReDelegateTxWithTarget failed",
			"pool", poolAddr.String(),
			"old validator", needRedelegateVals,
			"new validator", w.conn.validatorTargets[0],
			"err", err)
		return false
	}
	...
	sigBts, err := client.SignMultiSigRawTxWithSeq(seq, unSignedTx, poolClient.GetSubKeyName())
	if err != nil {
		w.log.Error("processValidatorRedelegateTarget SignMultiSigRawTx failed",
			"pool address", poolAddr.String(),
			"unsignedTx", string(unSignedTx),
			"err", err)
		return false
	...
	w.log.Info("processValidatorRedelegateTarget gen unsigned redelegate Tx",
		"pool address", poolAddr.String(),
		"proposalId", proposalIdHexStr)
