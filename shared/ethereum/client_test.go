...
func TestConnect(t *testing.T) {
	conn := NewClient(TestEndpoint, false, AliceKp, log15.Root(), GasLimit, MaxGasPrice)
	err := conn.Connect()
	if err != nil {
		t.Fatal(err)
	}
...
func TestConnection_SafeEstimateGas(t *testing.T) {
	...
	conn := NewClient(TestEndpoint, false, AliceKp, log15.Root(), GasLimit, MaxGasPrice.Add(MaxGasPrice, big.NewInt(1)))
	err := conn.Connect()
	if err != nil {
		t.Fatal(err)
	}
...
func TestConnection_SafeEstimateGasMax(t *testing.T) {
	...
	conn := NewClient(TestEndpoint, false, AliceKp, log15.Root(), GasLimit, maxPrice)
	err := conn.Connect()
	if err != nil {
		t.Fatal(err)
	}
